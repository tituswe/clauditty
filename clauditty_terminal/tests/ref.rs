...
use clauditty_terminal::event::{Event, EventListener};
use clauditty_terminal::grid::{Dimensions, Grid};
use clauditty_terminal::index::{Column, Line};
use clauditty_terminal::term::cell::Cell;
use clauditty_terminal::term::test::TermSize;
use clauditty_terminal::term::{Config, Term};
use clauditty_terminal::vte::ansi;
...
fn ref_test(dir: &Path) {
    let recording = read_u8(dir.join("clauditty.recording"));
