...
use clauditty_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use clauditty_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main Clauditty executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed Clauditty within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/clauditty/clauditty.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\clauditty\clauditty.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/clauditty/clauditty.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: Clauditty].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: Clauditty].
...
/// Send a message to the Clauditty socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same Clauditty process.
    CreateWindow(WindowOptions),

    /// Update the Clauditty configuration.
    Config(IpcConfig),

    /// Read runtime Clauditty configuration.
...
pub struct IpcConfig {
    ...
    #[clap(short, long, allow_hyphen_values = true, env = "CLAUDITTY_WINDOW_ID")]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "clauditty.bash"),
            (Shell::Fish, "clauditty.fish"),
            (Shell::Zsh, "_clauditty"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "clauditty", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/clauditty.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "clauditty", &mut file);
        // let mut file = File::create("../extra/completions/clauditty.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "clauditty", &mut file);
        // let mut file = File::create("../extra/completions/_clauditty").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "clauditty", &mut file);
    }
}
