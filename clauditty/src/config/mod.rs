...
pub fn deserialize_config(path: &Path, warn_pruned: bool) -> Result<Value> {
    ...
    if (extension == "yaml" || extension == "yml") && !contents.trim().is_empty() {
        warn!(
            "YAML config {path:?} is deprecated, please migrate to TOML using `clauditty migrate`"
        );
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("clauditty.{suffix}");
    ...
    xdg::BaseDirectories::with_prefix("clauditty")
        ...
        .or_else(|| {
            if let Ok(home) = env::var("HOME") {
                // Fallback path: $HOME/.config/clauditty/clauditty.toml.
                let fallback = PathBuf::from(&home).join(".config/clauditty").join(&file_name);
                if fallback.exists() {
                    return Some(fallback);
                }
                // Fallback path: $HOME/.clauditty.toml.
            ...
            let fallback = PathBuf::from("/etc/clauditty").join(&file_name);
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("clauditty.{suffix}");
    dirs::config_dir().map(|path| path.join("clauditty").join(file_name)).filter(|new| new.exists())
}
