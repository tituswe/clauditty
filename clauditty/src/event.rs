...
use clauditty_terminal::event::{Event as TerminalEvent, EventListener, Notify};
use clauditty_terminal::event_loop::Notifier;
use clauditty_terminal::grid::{BidirectionalIterator, Dimensions, Scroll};
use clauditty_terminal::index::{Boundary, Column, Direction, Line, Point, Side};
use clauditty_terminal::selection::{Selection, SelectionType};
use clauditty_terminal::term::cell::Flags;
use clauditty_terminal::term::search::{Match, RegexSearch};
use clauditty_terminal::term::{self, ClipboardType, Term, TermMode};
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn spawn_new_instance(&mut self) {
        ...
        let clauditty = env_args.next().unwrap();

        let mut args: Vec<String> = Vec::new();

        // Reuse the arguments passed to Clauditty for the new instance.
