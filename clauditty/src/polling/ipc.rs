...
const CLAUDITTY_SOCKET_ENV: &str = "CLAUDITTY_SOCKET";
...
impl IpcListener {
    pub fn new(
        ...
        // Register socket path as environment variable for `clauditty msg`.
        unsafe { env::set_var(CLAUDITTY_SOCKET_ENV, path.as_os_str()) };
        if options.daemon {
            println!("CLAUDITTY_SOCKET={}; export CLAUDITTY_SOCKET", path.display());
        }
...
/// Send a message to the active Clauditty socket.
...
pub fn socket_dir() -> PathBuf {
    xdg::BaseDirectories::with_prefix("clauditty")
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(CLAUDITTY_SOCKET_ENV) {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't Clauditty sockets.
...
pub fn socket_prefix() -> String {
    ...
    format!("Clauditty-{}", display.replace('/', "-"))
}

/// File prefix matching all available sockets.
#[cfg(target_os = "macos")]
pub fn socket_prefix() -> String {
    String::from("Clauditty")
}
