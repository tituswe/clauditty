...
use clauditty_terminal::term::{
    self, LineDamageBounds, MIN_COLUMNS, MIN_SCREEN_LINES, Term, TermDamage, TermMode,
};
use clauditty_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    pub fn new(
        ...
        // Always focus new windows, even if no Clauditty window is currently focused.
        #[cfg(target_os = "macos")]
        window.focus_window();
    ...
    pub fn end_frame(&mut self, scheduler: &mut Scheduler) {
        ...
        if matches!(self.raw_window_handle, RawWindowHandle::Xcb(_) | RawWindowHandle::Xlib(_)) {
            ...
            // will block to synchronize (this is `glClear` in Clauditty), which causes a
