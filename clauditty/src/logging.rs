...
pub const LOG_TARGET_IPC_CONFIG: &str = "clauditty_log_window_config";

/// Name for the environment variable containing the log file's path.
const CLAUDITTY_LOG_ENV: &str = "CLAUDITTY_LOG";

/// Logging target for config error messages.
pub const LOG_TARGET_CONFIG: &str = "clauditty_config_derive";

/// Logging target for winit events.
pub const LOG_TARGET_WINIT: &str = "clauditty_winit_event";

/// Name for the environment variable containing extra logging targets.
///
/// The targets are semicolon separated.
const CLAUDITTY_EXTRA_LOG_TARGETS_ENV: &str = "CLAUDITTY_EXTRA_LOG_TARGETS";
...
fn extra_log_targets() -> &'static [String] {
    ...
    EXTRA_LOG_TARGETS.get_or_init(|| {
        env::var(CLAUDITTY_EXTRA_LOG_TARGETS_ENV)
            .map_or(Vec::new(), |targets| targets.split(';').map(ToString::to_string).collect())
    })
}

/// List of targets which will be logged by Clauditty.
const ALLOWED_TARGETS: &[&str] = &[
    LOG_TARGET_IPC_CONFIG,
    LOG_TARGET_CONFIG,
    LOG_TARGET_WINIT,
    "clauditty_config_derive",
    "clauditty_terminal",
    "clauditty",
    "crossfont",
];
...
impl Logger {
    ...
    fn message_bar_log(&self, record: &log::Record<'_>, logfile_path: &str) {
        ...
        #[cfg(not(windows))]
        let env_var = format!("${CLAUDITTY_LOG_ENV}");
        #[cfg(windows)]
        let env_var = format!("%{}%", CLAUDITTY_LOG_ENV);
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        path.push(format!("Clauditty-{}.log", process::id()));

        // Set log path as an environment variable.
        unsafe { env::set_var(CLAUDITTY_LOG_ENV, path.as_os_str()) };
