...
This document gives an overview over Clauditty's features beyond its terminal
emulation capabilities. To get a list with supported control sequences take a
look at the [clauditty-escapes(7) manpage].

[clauditty-escapes(7) manpage]: ../extra/man/clauditty-escapes.7.scd

## Vi Mode

The vi mode allows moving around Clauditty's viewport and scrollback using the
...
Search allows you to find anything in Clauditty's scrollback buffer. You can
...
either feeds them to an external application or triggers one of Clauditty's
built-in actions.
...
Clauditty supports running multiple terminal emulators from the same Clauditty
instance. New windows can be created either by using the `CreateNewWindow`
keybinding action, or by executing the `clauditty msg create-window` subcommand.
